...
- If a diagram has **clear nodes and connections** (org chart, flowchart, process diagram, decision tree, deal structure, relationship map), output BOTH:
  ...
  2. A **Mermaid code block** representing the structure.
...
- Node IDs must be simple alphanumeric: `A`, `B1`, `node1` — no hyphens, spaces, or special chars in IDs.
- ALL node labels MUST be in double-quoted square brackets: `A["Label text here"]`
- **No parentheses** in labels — use fullwidth instead: `（` and `）` instead of `(` and `)`
- **No line breaks** inside labels — every node definition must be on a single line.
- Keep labels short (under 40 chars). If a label is long, abbreviate it.
- `subgraph` and `end` must each be on their own line.
- Example:
  ```mermaid
  graph TD
...
Transcribe all pages of the document provided.',
  true,
  'v8: Strict mermaid syntax rules — quoted labels in ["..."], no parentheses (use fullwidth), no line breaks in labels, short labels, alphanumeric IDs only, subgraph/end on own lines.'
);
